...
func TestLoadServerConfigFromFlags(t *testing.T) {
	...
	if err := flags.Parse([]string{
		...
		"--node-snapshot-interval", "3s",
		"--node-replication-outbox-path", "replication-outbox.jsonl",
...
func assertConfigValues(t *testing.T, cfg serverConfig) {
	...
	checks := []struct {
		...
		{name: "snapshot interval", got: cfg.Node.Snapshot.Interval, want: 3 * time.Second},
		{name: "replication outbox path", got: cfg.Node.Replication.Outbox.Path, want: "replication-outbox.jsonl"},
